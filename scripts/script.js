...
function fraseAleatoriaGanadora() {
    ...
    let frasesGanadoras = [
        ...
        `Con un movimiento certero, <b>${nombreJugador}</b> utiliza su <b>${jugadorArma}</b> para superar al <b>Esqueleto Infernal</b>. El monstruo se desmorona ante el impacto, mientras ${nombreJugador}se erige como una verdadera fuerza en esta encarnizada batalla. <b>GANASTE</b>`,
    ]
...
function fraseAleatoriaPerdedora() {
    ...
    let frasesPerdedoras = [
        ...
        `<b>${nombreJugador}</b> desata la fuerza de la <b>${jugadorArma}</b> contra el malévolo <b>Esqueleto Infernal</b>. Sin embargo, el Esqueleto Infernal se defiende hábilmente con su <b>${computadoraArma}</b>. ${nombreJugador} lucha valientemente, pero finalmente sufre una derrota. <b>PIERDES</b>`,
        ...
        `<b>${nombreJugador}</b> empuña la <b>${jugadorArma}</b> con destreza frente al imponente <b>Esqueleto Infernal</b>. Sin embargo, el Esqueleto Infernal contraataca con su letal <b>${computadoraArma}</b>. A pesar de la feroz lucha de ${nombreJugador} sufre una derrota en esta batalla intensa. <b>PIERDES</b>`,

        `<b>${nombreJugador}</b> libera su ataque más poderoso con la <b>${jugadorArma}</b>. Ambos combatientes desatan sus mejores habilidades, pero el <b>Esqueleto Infernal</b> se muestra implacable con su letal <b>${computadoraArma}</b>. A pesar de la valentía de ${nombreJugador}, su derrota se hace inevitable. <b>PIERDES</b>`,
...
function fraseAleatoriaEmpate() {
    botonAtacarHabilitar()
    let frasesEmpate = [
        `<b>${nombreJugador}</b> y el <b>Esqueleto Infernal</b> desatan el poderío de la <b>${jugadorArma}</b>. Sus ataques chocan con una fuerza arrolladora, pero ninguno de ellos logra prevalecer. La batalla termina en un empate, dejando a ambos luchadores exhaustos pero determinados a continuar la lucha. <b>EMPATE</b>.`,

        `<b>${nombreJugador}</b> y el <b>Esqueleto Infernal</b> se enfrentan usando la <b>${jugadorArma}</b>. Sus movimientos son rápidos y precisos, pero ninguno de ellos logra obtener la ventaja sobre el otro. La batalla termina en un empate, dejando a ambos con la determinación de superarse en el siguiente encuentro. <b>EMPATE</b>.`,

        `<b>${nombreJugador}</b> y el <b>Esqueleto Infernal</b> se enfrentan con la <b>${jugadorArma}</b>. Los filos chocan en un duelo impresionante, pero ninguno logra ganar la batalla. El enfrentamiento termina en un empate, dejando a ambos guerreros con la sensación de que aún hay mucho por demostrar. <b>EMPATE</b>.`,
